...
<template>
  <div class="flex flex-row mx-auto max-w-3xl">
    <Combobox
      v-slot="{ activeOption }"
      as="div"
      class="basis-2/3 mr-4 transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black ring-opacity-5 transition-all"
    ...
    <Combobox
      v-slot="{ activeOption }"
      as="div"
      class="basis-1/3 transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black ring-opacity-5 transition-all"
      @update:model-value="onSelect"
    >
      <div class="relative">
        <SearchIcon class="pointer-events-none absolute top-3.5 left-4 h-5 w-5 text-gray-400" aria-hidden="true" />
